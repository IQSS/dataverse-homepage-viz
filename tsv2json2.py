...
with open('data/root.tsv', newline='') as tsvfile:
#with open('data/all.tsv', newline='') as tsvfile:
...
for corkey, corval in data.items():
    ...
    for gcorkey, gcorval in corval.items():
        ...
        level2 = {}
        level2['children'] = []
        # level2 for children of root
        level2['date'] = dates[gcorkey]
        timestamp_format2 = '%Y-%m-%d %H:%M:%S'
        t1a = datetime.strptime(oldest_date, timestamp_format)
        try:
            t2a = datetime.strptime(dates[gcorkey], timestamp_format)
        except:
            t2a = datetime.strptime(dates[gcorkey], timestamp_format2)
        days2 = t2a - t1a
        level2['diff'] = days2.days
